...
@implementation CWColorSlider {
    UIVisualEffectView *_grabberEffectView;
    CWGradientView *_trackGradientView;
...
- (void)configureWithType:(CWSliderType)type color:(UIColor *)color {
    ...
    _grabberEffectView = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleSystemChromeMaterial]];
    _grabberEffectView.userInteractionEnabled = NO;
    _grabberEffectView.maskView = grabber;
    [self addSubview:_grabberEffectView];
    
    _trackGradientView = [[CWGradientView alloc] initWithColors:@[]];
...
- (void)setColor:(UIColor *)color {
    _colorValue = color;
    if (_type != CWSliderTypeHue || _trackGradientView.colors.count < 2) {
